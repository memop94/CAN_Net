...
void vfnGPIO_Init_channel(uint8_t channel, uint8_t input_output, uint8_t Open_drain)
{
    ...
    else if (input_output == GPIO_ADC)
    {
    	SIU.PCR[channel].R = EN_ANALOG_INP;
    }
}
...
void vfnGPIO_LED_Init(void)
{
	...
	vfnGPIO_Init_channel(ADC_D_SB,GPIO_ADC,GPIO_OPEN_DRAIN_DISABLE);
	vfnGPIO_Init_channel(ADC_P_SB,GPIO_ADC,GPIO_OPEN_DRAIN_DISABLE);
	vfnGPIO_Init_channel(ADC_P_SS,GPIO_ADC,GPIO_OPEN_DRAIN_DISABLE);
}
